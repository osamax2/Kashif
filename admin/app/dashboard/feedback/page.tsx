...
const STATUS_CONFIG = {
  new: { icon: AlertCircle, color: 'text-blue-600', bg: 'bg-blue-100', label_en: 'New', label_ar: 'جديد', label_ku: 'Nu' },
  in_progress: { icon: Clock, color: 'text-yellow-600', bg: 'bg-yellow-100', label_en: 'In Progress', label_ar: 'قيد المعالجة', label_ku: 'Li peshvecune' },
  resolved: { icon: CheckCircle2, color: 'text-green-600', bg: 'bg-green-100', label_en: 'Resolved', label_ar: 'تم الحل„', label_ku: 'Chareserkiri' },
  dismissed: { icon: XCircle, color: 'text-gray-500', bg: 'bg-gray-100', label_en: 'Dismissed', label_ar: 'مرفوض', label_ku: 'Betalkiri' },
};

function getCategoryLabel(cat: string, isRTL: boolean, language?: string) {
  const labels: Record<string, { en: string; ar: string; ku: string }> = {
    bug: { en: 'Bug', ar: 'خلل', ku: 'Têkçûn' },
    suggestion: { en: 'Suggestion', ar: 'اقتراح', ku: 'Pêşniyar' },
    complaint: { en: 'Complaint', ar: 'شكوى', ku: 'şikayet' },
    other: { en: 'Other', ar: 'أخرى', ku: 'Yên din' },
  };
  if (language === 'ku') return labels[cat]?.ku || cat;
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${catConfig.bg} ${catConfig.color}`}>
              ...
              {getCategoryLabel(fb.category, isRTL, language)}
            </span>
              ...
              <StatusIcon className="w-3 h-3" />
                {language === 'ku' ? statusConfig.label_ku : isRTL ? statusConfig.label_ar : statusConfig.label_en}
            </span>
            </div>
  ...
  return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 sticky top-4">
        <div className="p-4 border-b border-gray-200">
          ...
          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${catConfig.bg} ${catConfig.color}`}>
            <CatIcon className="w-3 h-3" />
            {getCategoryLabel(feedback.category, isRTL, language)}
          </span>
        ...
        <div className="p-4 space-y-4">
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">{language === 'ar' ? 'المستخدم' : language === 'ku' ? 'Bikarhêner' : 'User'}</p>
            <p className="text-sm font-medium text-gray-900">{user?.full_name || `User #${feedback.user_id}`}</p>
            <p className="text-xs text-gray-500">{user?.email || ''}</p>
          </div>

          <div>
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">{language === 'ar' ? 'الرسالة' : language === 'ku' ? 'Peyam' : 'Message'}</p>
            <p className="text-sm text-gray-700 whitespace-pre-wrap bg-gray-50 p-3 rounded-lg">{feedback.message}</p>
          </div>

          <div>
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">{language === 'ar' ? 'التاريخ' : language === 'ku' ? 'Dîrok' : 'Date'}</p>
            <p className="text-sm text-gray-700">{formatDate(feedback.created_at, isRTL)}</p>
          </div>

          <div>
            <p className="text-xs font-medium text-gray-500 uppercase mb-2">{language === 'ar' ? 'تحديث الحالة' : language === 'ku' ? 'Rewş nûve bike' : 'Update Status'}</p>
              ...
              {Object.entries(STATUS_CONFIG).map(([key, config]) => {
                ...
                return (
                    ...
                    >
                      ...
                      {language === 'ku' ? config.label_ku : isRTL ? config.label_ar : config.label_en}
                    </button>
                );
              })}
            </div>
          ...
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">{language === 'ar' ? 'ملاحظات المشرف' : language === 'ku' ? 'Nivîsên admin' : 'Admin Notes'}</p>
            <textarea
                value={adminNotes}
                onChange={(e) => onAdminNotesChange(e.target.value)}
                placeholder={language === 'ar' ? 'اكتب ملاحظاتك هنا...' : language === 'ku' ? 'Nivîsên xwe li vir binivîse...' : 'Write your notes here...'}
                rows={3}
            ...
            >
              ...
              {language === 'ar' ? 'حفظ الملاحظات' : language === 'ku' ? 'Nivîsanan tomar bike' : 'Save Notes'}
            </button>
          </div>
        </div>
      </div>
...
/* Stats cards */
function StatsCards({ isRTL, language, counts, statusFilter, onToggleFilter }: Readonly<{
  isRTL: boolean;
  language?: string;
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <button type="button" className="bg-blue-50 border border-blue-200 rounded-xl p-4 cursor-pointer hover:shadow-md transition text-left" onClick={() => onToggleFilter('new')}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-blue-800">{language === 'ar' ? 'جديد' : language === 'ku' ? 'Nû' : 'New'}</span>
          </div>
        ...
        <button type="button" className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 cursor-pointer hover:shadow-md transition text-left" onClick={() => onToggleFilter('in_progress')}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-yellow-800">{language === 'ar' ? 'قيد المعالجة' : language === 'ku' ? 'Li pêşveçûn' : 'In Progress'}</span>
          </div>
        ...
        <button type="button" className="bg-green-50 border border-green-200 rounded-xl p-4 cursor-pointer hover:shadow-md transition text-left" onClick={() => onToggleFilter('resolved')}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-green-800">{language === 'ar' ? 'تم الحل' : language === 'ku' ? 'Çareserkirî' : 'Resolved'}</span>
          </div>
        ...
        <button type="button" className="bg-gray-50 border border-gray-200 rounded-xl p-4 cursor-pointer hover:shadow-md transition text-left" onClick={() => onToggleFilter('dismissed')}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-gray-700">{language === 'ar' ? 'مرفوض\'' : language === 'ku' ? 'Betalkirî' : 'Dismissed'}</span>
          </div>
...
/* Filters bar */
function FiltersBar({ isRTL, language, searchQuery, categoryFilter, statusFilter, onSearchChange, onCategoryChange, onClear }: Readonly<{
  isRTL: boolean;
  language?: string;
  searchQuery: string;
  ...
  return (
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder={language === 'ar' ? 'بحث في الملاحظات...' : language === 'ku' ? 'Di nerînan de bigere...' : 'Search feedback...'} value={searchQuery} onChange={(e) => onSearchChange(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent" />
        </div>
        <div className="flex items-center gap-2">
          ...
          <select value={categoryFilter} onChange={(e) => onCategoryChange(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary text-sm">
            <option value="">{language === 'ar' ? 'كل الفئات' : language === 'ku' ? 'Hemû kategoriyan' : 'All Categories'}</option>
            <option value="bug">{language === 'ar' ? 'خلل' : language === 'ku' ? 'Têkçûn' : 'Bug'}</option>
            <option value="suggestion">{language === 'ar' ? 'اقتراح' : language === 'ku' ? 'Pêşniyar' : 'Suggestion'}</option>
            <option value="complaint">{language === 'ar' ? 'شكوى' : language === 'ku' ? 'Gilî' : 'Complaint'}</option>
            <option value="other">{language === 'ar' ? 'أخرى' : language === 'ku' ? 'Yên din' : 'Other'}</option>
          </select>
        </div>
        {hasFilters && (
            <button onClick={onClear} className="px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition">
              {language === 'ar' ? 'مسح الفلاتر' : language === 'ku' ? 'Parzûnan paqij bike' : 'Clear Filters'}
            </button>
        )}
      </div>
  );
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-gray-900">
              {language === 'ar' ? 'ملاحظات المستخدمين' : language === 'ku' ? 'Nerînên bikarhêneran' : 'User Feedback'}
            </h1>
          </div>
          <button
              onClick={loadData}
          ...
          >
            ...
            {language === 'ar' ? 'تحديث' : language === 'ku' ? 'Nûve bike' : 'Refresh'}
          </button>
        </div>

        <StatsCards isRTL={isRTL} language={language} counts={counts} statusFilter={statusFilter} onToggleFilter={toggleStatusFilter} />
        <FiltersBar isRTL={isRTL} language={language} searchQuery={searchQuery} categoryFilter={categoryFilter} statusFilter={statusFilter} onSearchChange={setSearchQuery} onCategoryChange={setCategoryFilter} onClear={clearFilters} />
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
                <p className="text-sm text-gray-600">
                  {language === 'ar' ? `${filteredFeedbacks.length} ملاحظة من ${feedbacks.length}` : language === 'ku' ? `${filteredFeedbacks.length} ji ${feedbacks.length} nerîn` : `${filteredFeedbacks.length} of ${feedbacks.length} feedback items`}
                </p>
              </div>
              {filteredFeedbacks.length === 0 ? (
                  <div className="p-8 text-center text-gray-500">
                    <MessageSquareText className="w-12 h-12 mx-auto mb-3 text-gray-300" />
                    <p>{language === 'ar' ? 'لا توجد ملاحظات' : language === 'ku' ? 'Tîştekî nehatî qeyd kirin' : 'No feedback found'}</p>
                  </div>
              ) : (
          ...
          <div className="lg:col-span-1">
            {selectedFeedback ? (
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center text-gray-400">
                  ...
                  <p className="text-sm">
                    {language === 'ar' ? 'اختر ملاحظة لعرض التفاصيل' : language === 'ku' ? 'Nerînek hilbijêre da ku hûrgulî bibînî' : 'Select a feedback item to view details'}
                  </p>
                </div>
            )}
          </div>
