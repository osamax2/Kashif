...
export default function CouponsPage() {
  ...
  const handleRestoreCoupon = async (couponId: number) => {
    try {
      ...
      alert(language === 'ar' ? 'فشل في استعادة الكوبون' : language === 'ku' ? 'Vegera kuponê têk çû' : 'Failed to restore coupon');
    }
  };

  const handlePermanentDelete = async (couponId: number) => {
    if (!confirm(language === 'ar' ? 'هل أنت متأكد من الحذف النهائي؟ لا يمكن التراجع عن هذا الإجراء.' : language === 'ku' ? 'Tu ji jêbirina mayînde piştrast in? Ev çalakî nikare were betalkirin.' : 'Are you sure you want to permanently delete? This action cannot be undone.')) {
      return;
    }
    try {
  ...
  const handleCreate = async () => {
    ...
    if (!formData.name || !formData.name.trim()) {
      errors.name = language === 'ar' ? 'اسم الكوبون مطلوب' : language === 'ku' ? 'Navê kuponê pêwîst e' : 'Coupon name is required';
    }
    if (!formData.description || !formData.description.trim()) {
      errors.description = language === 'ar' ? 'وصف الكوبون مطلوب' : language === 'ku' ? 'Danasîna kuponê pêwîst e' : 'Description is required';
    }
    if (!formData.company_id) {
      errors.company_id = language === 'ar' ? 'الشركة مطلوبة' : language === 'ku' ? 'Şirkêt pêwîst e' : 'Company is required';
    }
    if (!formData.points_cost && formData.points_cost !== '0') {
      errors.points_cost = language === 'ar' ? 'تكلفة النقاط مطلوبة' : language === 'ku' ? 'Nirxa xalên pêwîst e' : 'Points cost is required';
    }
    ...
    if (!errors.points_cost && (isNaN(pointsCost) || pointsCost < 0 || pointsCost > 100000)) {
      errors.points_cost = language === 'ar' ? 'تكلفة النقاط يجب أن تكون بين 0 و 100000' : language === 'ku' ? 'Nirxa xalan divê di navbera 0 û 100000 de be' : 'Points cost must be between 0 and 100,000';
    }
    ...
    try {
      ...
      alert(language === 'ar' ? 'تم إنشاء الكوبون بنجاح!' : language === 'ku' ? 'Kupon bi serkeftin hate çêkirin!' : 'Coupon created successfully!');
      ...
      if (error.response?.status !== 401) {
        alert(language === 'ar' ? 'فشل إنشاء الكوبون' : language === 'ku' ? 'Çêkirina kuponê têk çû' : 'Failed to create coupon');
      }
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      alert(language === 'ar' ? 'تم تحديث الكوبون بنجاح!' : language === 'ku' ? 'Kupon bi serkeftin hate nûkirin!' : 'Coupon updated successfully!');
      ...
      if (error.response?.status !== 401) {
        alert(language === 'ar' ? 'فشل تحديث الكوبون' : language === 'ku' ? 'Nûkirina kuponê têk çû' : 'Failed to update coupon');
      }
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert(language === 'ar' ? 'تم حذف الكوبون بنجاح!' : language === 'ku' ? 'Kupon bi serkeftin hate jêbirin!' : 'Coupon deleted successfully!');
      ...
      if (error.response?.status !== 401) {
        alert(language === 'ar' ? 'فشل حذف الكوبون' : language === 'ku' ? 'Jêbirina kuponê têk çû' : 'Failed to delete coupon');
      }
    }
  };
      ...
      <div dir={isRTL ? 'rtl' : 'ltr'}>
        <div className={`mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 ${isRTL ? 'sm:flex-row-reverse' : ''}`}>
          ...
          <div className={`flex flex-wrap gap-2 sm:gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            >
              ...
              <span className="hidden xs:inline">{language === 'ar' ? 'المحذوفات' : language === 'ku' ? 'Jêbirî' : 'Trash'}</span>
            </button>
            <button
                onClick={() => {
                  resetForm();
            ...
            >
              ...
              <span className="xs:hidden">{language === 'ar' ? 'إضافة' : language === 'ku' ? 'Zêde bike' : 'Add'}</span>
            </button>
          </div>
        </div>
        ...
        <div className="mb-4">
          <input
              ...
              placeholder={language === 'ar' ? 'ابحث عن الكوبونات...' : language === 'ku' ? 'Li kuponan bigere...' : 'Search coupons...'}
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 ${isRTL ? 'text-right' : ''}`}>
          ...
          <div>
            <label className={`block text-sm font-medium text-gray-700 mb-1 ${isRTL ? 'text-right' : ''}`}>
              {t.coupons.category || (language === 'ar' ? 'الفئة' : language === 'ku' ? 'Kategorî' : 'Category')}
            </label>
            <select
            ...
            >
              <option value="">{language === 'ar' ? 'كل الفئات' : language === 'ku' ? 'Hemû kategorî' : 'All Categories'}</option>
          ...
          {!isCompanyUser && (
              <div>
                <label className={`block text-sm font-medium text-gray-700 mb-1 ${isRTL ? 'text-right' : ''}`}>
                  {t.coupons.couponCompany || (language === 'ar' ? 'الشركة' : language === 'ku' ? 'Şirkêt' : 'Company')}
                </label>
                <select
                    value={filterCompany}
                    ...
                    className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary ${isRTL ? 'text-right' : ''}`}
                >
                  <option value="">{language === 'ar' ? 'كل الشركات' : language === 'ku' ? 'Hemû şirkêt' : 'All Companies'}</option>
          ...
          <div>
            <label className={`block text-sm font-medium text-gray-700 mb-1 ${isRTL ? 'text-right' : ''}`}>
              {t.common.status || (language === 'ar' ? 'الحالة' : language === 'ku' ? 'Rewş' : 'Status')}
            </label>
            <select
                value={filterStatus}
                ...
                className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary ${isRTL ? 'text-right' : ''}`}
            >
              <option value="">{language === 'ar' ? 'كل الحالات' : language === 'ku' ? 'Hemû rewş' : 'All Status'}</option>
              <option value="ACTIVE">{t.common.active || (language === 'ar' ? 'نشط' : language === 'ku' ? 'Çalak' : 'Active')}</option>
              <option value="INACTIVE">{t.common.inactive || (language === 'ar' ? 'غير نشط' : language === 'ku' ? 'Neçalak' : 'Inactive')}</option>
              <option value="EXPIRED">{t.common.expired || (language === 'ar' ? 'منتهي الصلاحية' : language === 'ku' ? 'Demborî' : 'Expired')}</option>
            </select>
          </div>
        </div>
        ...
        {(searchQuery || filterCategory || filterCompany || filterStatus) && (
            <div className={`mb-4 flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
              <p className="text-sm text-gray-500">
                {language === 'ar' ? `تم العثور على ${filteredCoupons.length} من ${coupons.length} كوبون` : language === 'ku' ? ` ${filteredCoupons.length} ji ${coupons.length} kupon hatin dîtin` : `Found ${filteredCoupons.length} of ${coupons.length} coupons`
                }
              </p>
              <button
                  onClick={() => {
              ...
              >
                {language === 'ar' ? 'مسح الفلاتر' : language === 'ku' ? 'Fîltiran paqij bike' : 'Clear Filters'}
              </button>
            </div>
        )}
        ...
        {filteredCoupons.length === 0 && coupons.length > 0 && (searchQuery || filterCategory || filterCompany || filterStatus) && (
            <div className="text-center py-12">
              <p className="text-gray-500">
                {language === 'ar' ? 'لم يتم العثور على كوبونات مطابقة للفلاتر' : language === 'ku' ? 'Li gorî fîltiran kupon nehatin dîtin' : 'No coupons found matching your filters'}
              </p>
            </div>
        )}
              ...
              <div className="bg-white rounded-xl p-6 w-full max-w-4xl max-h-[80vh] overflow-hidden flex flex-col" dir={isRTL ? 'rtl' : 'ltr'}>
                <div className={`flex justify-between items-center mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <h2 className={`text-xl font-bold text-gray-900 ${isRTL ? 'text-right' : ''}`}>
                    {language === 'ar' ? 'الكوبونات المحذوفة' : language === 'ku' ? 'Kuponên jêbirî' : 'Deleted Coupons'}
                  </h2>
                  <button
                ...
                <div className="flex-1 overflow-y-auto">
                  {trashLoading ? (
                      ...
                      <div className="text-center py-12 text-gray-500">
                        ...
                        <p>{language === 'ar' ? 'لا توجد كوبونات محذوفة' : language === 'ku' ? 'Kuponên jêbirî tune ne' : 'No deleted coupons'}</p>
                      </div>
                  ) : (
                              ...
                              <div key={coupon.id} className="border rounded-lg p-4 bg-gray-50">
                                ...
                                <div className={`flex gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                  ...
                                  >
                                    ...
                                    {language === 'ar' ? 'استعادة' : language === 'ku' ? 'Vegerîne' : 'Restore'}
                                  </button>
                                  <button
                                  ...
                                  >
                                    ...
                                    {language === 'ar' ? 'حذف نهائي' : language === 'ku' ? 'Jêbirina herdemî' : 'Delete Forever'}
                                  </button>
                                </div>
                              </div>
                          );
...
function CouponForm({
  ...
  return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          {validationErrors.points_cost
              ...
              : <p className={`text-xs text-gray-500 mt-1 ${isRTL ? 'text-right' : ''}`}>
                {language === 'ar' ? 'أدخل رقم بين 0 و 100000 (أرقام إنجليزية فقط)' : language === 'ku' ? 'Hejmara di navbera 0 û 100000 de binivîse (tenê jimareyên Îngilîzî)' : 'Enter a number between 0 and 100,000 (English numbers only)'}
              </p>
          }
        </div>
        ...
        {!isCompanyUser && (
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              ...
              <input
                  ...
                  placeholder={language === 'ar' ? 'ابحث عن شركة...' : language === 'ku' ? 'Li şirkêtê bigere...' : 'Search for a company...'}
              ...
              {companySearch && filteredCompanies.length === 0 && showCompanySuggestions && (
                  <div className={`absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg px-4 py-2 text-gray-500 ${isRTL ? 'text-right' : ''}`}>
                    {language === 'ar' ? 'لم يتم العثور على شركات' : language === 'ku' ? 'Şirkêt nehatin dîtin' : 'No companies found'}
                  </div>
              )}
            </div>
        )}
        ...
        <div className="md:col-span-2">
          <label className={`block text-sm font-medium text-gray-700 mb-2 ${isRTL ? 'text-right' : ''}`}>{t.coupons.address || (language === 'ar' ? 'العنوان' : language === 'ku' ? 'Navnîşan' : 'Address')}</label>
          <input
              type="text"
              value={formData.address}
              onChange={(e) => setFormData({ ...formData, address: e.target.value })}
              className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary ${isRTL ? 'text-right' : ''}`}
              placeholder={language === 'ar' ? 'أدخل عنوان موقع الكوبون' : language === 'ku' ? 'Navnîşana cihê kuponê binivîse' : 'Enter coupon location address'}
          />
        </div>
        ...
        <div>
          ...
          <input
              ...
              className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary ${isRTL ? 'text-right' : ''}`}
              placeholder={language === 'ar' ? 'أرقام إنجليزية فقط' : language === 'ku' ? 'Tenê jimareyên Îngilîzî' : 'English numbers only'}
          />
        </div>
