      ...
      <div dir={isRTL ? 'rtl' : 'ltr'}>
        <div className={`mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 ${isRTL ? 'sm:flex-row-reverse' : ''}`}>
          <div className={isRTL ? 'text-right' : ''}>
            ...
            <p className="text-gray-600 mt-1 sm:mt-2 text-sm sm:text-base">{language === 'ar' ? 'إدارة فئات القسائم' : language === 'ku' ? 'Rêveberiya kategoriyên kupônan' : 'Manage coupon categories'}</p>
          </div>
          <button
              onClick={() => {
                resetForm();
        ...
        {categories.length === 0 && (
            <div className="text-center py-12">
              <p className="text-gray-500">{language === 'ar' ? 'لم يتم العثور على فئات' : language === 'ku' ? 'Rêveberiya kategoriyên kupônan' : 'No categories found'}</p>
            </div>
        )}
