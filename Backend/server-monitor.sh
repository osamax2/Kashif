...
check_crypto_miners() {
    ...
    local miners
    miners=$(ps aux | grep -i -E "xmrig|xmr-stak|minerd|minergate|cpuminer|ethminer|cgminer|bfgminer|systemp|kdevtmpfsi|kinsing" | grep -v grep | grep -v "server-monitor")
...
check_suspicious_files() {
    ...
    local cron_jobs
    cron_jobs=$(crontab -l 2>/dev/null | grep -v "^#" | grep -v "^$" | grep -v "monitor\.py" | grep -v "kashif-monitor" | grep -v "backup-kashif")
